    ...
    const handleSelect = (type: 'Archetype' | 'Role' | 'Genre' | 'Category', value: string) => {
        ...
        if (type === 'Archetype') {
            matches = characters
                ...
                .map(c => ({ id: c.id, name: c.name, url: `/characters/view?id=${encodeURIComponent(c.id)}`, type: 'Character', progress: c.progress }));
        } else if (type === 'Role') {
            matches = characters
            ...
            matches = worlds
                ...
                .map(w => ({ id: w.id, name: w.name, url: `/worlds/view?id=${encodeURIComponent(w.id)}`, type: 'World', progress: w.progress }));
        } else if (type === 'Category') {
            if (value === 'Characters') {
                matches = characters.map(c => ({ id: c.id, name: c.name, url: `/characters/view?id=${encodeURIComponent(c.id)}`, type: 'Character', progress: c.progress }));
            } else if (value === 'Worlds') {
                matches = worlds.map(w => ({ id: w.id, name: w.name, url: `/worlds/view?id=${encodeURIComponent(w.id)}`, type: 'World', progress: w.progress }));
            } else if (value === 'Projects') {
                matches = projects.map(p => ({ id: p.id, name: p.name, url: `/projects/view?id=${encodeURIComponent(p.id)}`, type: 'Project', progress: p.progress }));
            } else if (value === 'Global Progress') {
                matches = [
                    ...characters.map(c => ({ ...c, type: 'Character' as const, url: `/characters/view?id=${encodeURIComponent(c.id)}` })),
                    ...worlds.map(w => ({ ...w, type: 'World' as const, url: `/worlds/view?id=${encodeURIComponent(w.id)}` })),
                    ...projects.map(p => ({ ...p, type: 'Project' as const, url: `/projects/view?id=${encodeURIComponent(p.id)}` }))
                ];
